            ...
            >
                ...
                <div className={`p-4 border-t border-slate-200 ${isCollapsed ? 'flex flex-col gap-2 justify-center' : ''}`}>
                    <div className={`flex items-center gap-3 p-2 rounded-xl hover:bg-slate-50 cursor-pointer transition-colors ${isCollapsed ? 'justify-center p-0' : ''}`}>
                        <div className="w-10 h-10 rounded-full bg-slate-200 bg-center bg-cover shrink-0" style={{ backgroundImage: "url('https://lh3.googleusercontent.com/aida-public/AB6AXuDw5mwhe7jy2edaIS6zXjemObgNFmMEdbIgrF0lOg3q07bSmDuq3YoPOxoGGmZdRLcaPgOasAdHqnbWGar4vQdmVPgspxvsgwXt6lFX3pu9h3C636MPj7j_grZ_3Y6MWI_HVSR5gY6uLID7Z_HLNEVUJytTIXzIgnZrp9banoIjFJcrIa-c6bgz0ex1IeJAesOicTUwjMuD8uJnHnwIHMPgpC3kfCH20rw0nRhA2hEqMgVOUt57goo98PFourOlzWB-icnHtgDwbNA')" }}></div>
                        <div className={`flex flex-col ${isCollapsed ? 'hidden' : 'block'} whitespace-nowrap overflow-hidden`}>
                            <span className="text-sm font-semibold text-slate-900">Carlos Ruiz</span>
                    ...
                    <Link
                        href="/login"
                        className={`flex items-center gap-3 p-2 rounded-xl text-slate-600 hover:text-red-600 hover:bg-red-50 transition-colors group ${isCollapsed ? 'justify-center' : ''}`}
                        title="Cerrar Sesión"
                    >
