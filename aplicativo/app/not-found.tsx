        ...
        const handler = (e: MouseEvent) => {
            requestAnimationFrame(() => {
                ...
                setCoords(depths.map(depth => ({
                    x: (depth * x) / 2, // Reduced movement for subtlety
                    y: (depth * y) / 2,
                })));
            });
        };
...
const Canyon = () => {
    ...
    return (
        <svg viewBox='0 0 2000 720' className='bg-slate-50 absolute inset-0 w-full h-full object-cover pointer-events-none' preserveAspectRatio='xMidYMid slice'>
            ...
            <g style={{ transform: `translate(${p2.x}px,${p2.y}px)`, opacity: 0.5 }}>
            ...
            <g style={{ transform: `translate(${p3.x}px,${p3.y}px)`, opacity: 0.8 }}>
            ...
            {/* Blur shadow for depth */}
            <path style={{ transform: `translate(${p3.x}px,${p3.y}px)`, filter: 'blur(80px)' }} fill="rgba(100,116,139,0.2)" d='m-300,400 H2400 V700 H0 z' />

            {/* 404 Text - Brand Green inside the mountain */}
            <text style={{ transform: `translate(${p4.x}px,${p4.y}px)` }} x='1000' y='550' textAnchor='middle' fill="#13ec80" fontSize="660" fontWeight="800" filter="drop-shadow(0 0 50px rgba(19, 236, 128, 0.3))">404</text>

            {/* Foreground Mountain - Slightly Darker Gray */}
            <path style={{ transform: `translate(${p5.x}px,${p5.y}px)` }} className="fill-slate-300" d="m2195 396v531.1h-2437.2v-538.1l359.2 60l96-22l63 44l169-40l83 39l348-47l147 28l125-32l75 47l75-21l221 28l263-75l109 31z" />
            ...
            <g style={{ transform: `translate(${p6.x}px,${p6.y}px)`, filter: 'blur(2px)' }}>
                <Cactus x={100} y={180} scale={0.5} />
                <Cactus x={700} y={200} scale={0.55} />
                <Cactus x={1350} y={250} scale={0.5} />
            </g>

            <g style={{ transform: `translate(${p7.x}px,${p7.y}px)`, filter: 'blur(4px)' }}>
...
export default function NotFound() {
    return (
        <div className="bg-slate-50 w-screen h-screen flex flex-col items-center justify-center font-sans overflow-hidden text-slate-900 relative">
            <div className="relative z-10 text-center -mt-32 drop-shadow-xl p-8 bg-white/20 backdrop-blur-sm rounded-3xl border border-white/40">
                <h1 className="text-8xl font-black mb-2 tracking-tighter text-slate-800 font-display">
                    ¡Ups!
                </h1>
                <h2 className="text-3xl font-bold mb-4 text-slate-800">
                    Página no encontrada
                </h2>
                <p className="text-slate-500 font-medium tracking-wide text-lg mb-8 max-w-lg mx-auto leading-relaxed">
                    Parece que tomaste un desvío inesperado. Esta página no está en nuestros mapas. 🌵
                </p>
                <div className="flex gap-4 justify-center items-center">
                    <Link href="/login" className="px-6 py-3 bg-[#13ec80] hover:bg-[#0fd673] text-[#0d1b14] font-bold rounded-xl shadow-lg shadow-green-200 transition-all hover:scale-105">
                        Volver al inicio
                    </Link>
                </div>
            </div>
            ...
            <div className="absolute bottom-6 text-slate-400 text-xs tracking-[0.2em] font-bold z-10">
                DONTENDERO
            </div>
        </div>
    );
