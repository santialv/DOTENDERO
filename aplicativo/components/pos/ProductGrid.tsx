            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-4 pb-20">
                {products.map(product => (
                    <div
                        ...
                        className="bg-white p-3 rounded-2xl shadow-sm border border-slate-100 hover:shadow-lg hover:border-primary/30 cursor-pointer transition-all group flex flex-col h-full relative"
                    >

                        <div className="flex flex-col flex-1">
                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1 block">{product.brand}</span>
                            <h3 className="font-semibold text-slate-900 leading-tight mb-1 text-sm line-clamp-2">{product.name}</h3>
