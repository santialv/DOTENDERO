...
export function DonTenderoChat() {
    ...
    const [messages, setMessages] = useState<Message[]>([
        { role: "assistant", content: "¡Hola! Soy tu asistente de Don Tendero. Pregúntame qué es la app o cómo gestionar tu negocio." }
    ]);
    ...
    if (!isOpen) {
        return (
            <Button
                ...
                className="h-16 w-16 rounded-full shadow-[0_0_20px_rgba(19,236,128,0.4)] bg-[#13ec80] hover:bg-[#0eb562] text-slate-900 p-0 flex items-center justify-center transition-all hover:scale-110 active:scale-95 border-2 border-white"
            >
    ...
    return (
        <Card className="w-[350px] md:w-[380px] h-[500px] flex flex-col shadow-2xl rounded-2xl overflow-hidden border-0 animate-in slide-in-from-bottom-10 fade-in duration-200 p-0 gap-0">
            {/* Header */}
            <div className="bg-[#1e293b] p-4 flex items-center justify-between shrink-0">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="text-white font-bold text-sm">Don Tendero AI</h3>
                ...
                {messages.map((msg, idx) => (
                    ...
                    >
                        ...
                        >
                            ...
                            {/* Error Fallback / Support Button */}
                            {(msg.role === "assistant" && (msg.content.includes("problema") || msg.content.includes("error") || msg.content.includes("configuración"))) && (
                                <div className="mt-3 pt-3 border-t border-slate-200">
                                    <a
                                        href="https://wa.me/573107146415?text=Hola,%20tuve%20un%20error%20en%20el%20Chatbot%20de%20la%20web"
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="text-xs flex items-center gap-2 bg-[#25D366] text-white px-3 py-2 rounded-lg hover:bg-[#20bd5a] transition-colors font-bold no-underline w-fit"
                                    >
            ...
            <div className="p-4 bg-white border-t border-slate-100">
                <form onSubmit={handleSubmit} className="flex gap-2">
                    <Input
                        ...
                        className="rounded-xl bg-slate-50 border-slate-200 focus-visible:ring-[#13ec80] pl-4 h-11 text-slate-900"
                        autoFocus
                    />
                    <Button
                        type="submit"
